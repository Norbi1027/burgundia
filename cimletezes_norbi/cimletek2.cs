...
namespace CimletekOOP
{
    ...
    public class Penz
    {
        ...
        public List<Cimletek2> getCimleth()
        {
            return penzek;
        }

        public string setCimleth(int osszeg)
        {
            string cimlet = null;
            int maradék = osszeg;
            foreach (var pénznem in penzek)
            {
                if (maradék >= pénznem.cimleth)
                {
                    int darab = maradék / pénznem.cimleth;
                    maradék %= pénznem.cimleth;
                    if (darab > 0)
                    {
